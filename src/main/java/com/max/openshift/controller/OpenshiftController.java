...
public class OpenshiftController {
	...
	public ModelAndView getStats(@RequestParam String username) {
		Long conteggio = contattoService.conteggioContatti();
		List<Contatto> usercontatto = contattoService.findByUsername(username);
		ModelAndView mv = new ModelAndView();
		mv.setViewName("statpage");
		mv.addObject("conteggio", conteggio);
		mv.addObject("usercontatto", usercontatto);
		return mv;
	}
}
